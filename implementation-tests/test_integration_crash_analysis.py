...
from packages.binary_analysis.radare2_wrapper import Radare2Wrapper, is_radare2_available, format_disassembly_text
...
def radare2_wrapper(test_binary):
    """Create Radare2Wrapper directly (simulates what CrashAnalyser does)."""
...
class TestCrashAnalysisIntegration:
    ...
    def test_radare2_wrapper_initialization(self, radare2_wrapper):
        """
        INTEGRATION: Verify Radare2Wrapper initializes properly.

        Tests:
        - Radare2Wrapper.__init__() completes successfully
        - Radare2Wrapper is configured with proper defaults
        - Binary is loaded successfully
        """
        assert radare2_wrapper is not None
        assert radare2_wrapper.binary.exists()
        print(f"\n✓ Radare2Wrapper initialized with binary: {radare2_wrapper.binary}")

    def test_radare2_analysis_runs_successfully(self, radare2_wrapper):
        """
    ...
    def test_disassembly_at_address_flow(self, radare2_wrapper):
        """
        ...
        instructions = radare2_wrapper.disassemble_at_address(test_address, count=10)
    ...
    def test_decompilation_flow(self, radare2_wrapper):
        """
        ...
        try:
            decompiled = radare2_wrapper.decompile_function(test_address)
    ...
    def test_imports_for_canary_detection(self, radare2_wrapper):
        """
    ...
    def test_security_info_analysis(self, radare2_wrapper):
        """
        ...
        - Crypto usage
        """
        security = radare2_wrapper.get_security_info()
    ...
    def test_cross_references_analysis(self, radare2_wrapper):
        """
        ...
        xrefs_to = radare2_wrapper.get_xrefs_to(test_func.offset)
        print(f"\n✓ Function {test_func.name}:")
        print(f"  Called by: {len(xrefs_to)} locations")

        # Get references FROM this function (what it calls)
        xrefs_from = radare2_wrapper.get_xrefs_from(test_func.offset)
        print(f"  Calls: {len(xrefs_from)} locations")

    def test_call_graph_for_crash_context(self, radare2_wrapper):
        """
        ...
        call_graph = radare2_wrapper.get_call_graph(test_func.offset)
    ...
    def test_full_workflow_simulation(self, radare2_wrapper):
        """
        ...
        print(f"\n1. Binary: {radare2_wrapper.binary}")

        # Step 2: Analysis performed
        print("\n2. Running analysis...")
        assert radare2_wrapper.analyze()
        functions = radare2_wrapper.list_functions()
        ...
        # Disassembly
        instructions = radare2_wrapper.disassemble_at_address(crash_addr, count=5)
        print(f"   Disassembly: {len(instructions)} instructions")

        # Xrefs
        xrefs_to = radare2_wrapper.get_xrefs_to(crash_addr)
        print(f"   Cross-refs: {len(xrefs_to)} callers")

        # Call graph
        call_graph = radare2_wrapper.get_call_graph(crash_addr)
        print(f"   Call graph: {type(call_graph).__name__}")

        # Step 5: Format for analysis (simplified)
        print("\n5. Analysis context assembled:")
        print(f"   - Binary: {radare2_wrapper.binary.name}")
...
class TestPerformanceIntegration:
    ...
    def test_analysis_completes_within_timeout(self, radare2_wrapper):
        """
        ...
        assert elapsed < radare2_wrapper.timeout

        print(f"\n✓ Analysis completed in {elapsed:.2f}s (timeout: {radare2_wrapper.timeout}s)")

    def test_multiple_operations_work_together(self, radare2_wrapper):
        """
        ...
        if len(functions1) > 0:
            disasm = radare2_wrapper.disassemble_at_address(functions1[0].offset, count=5)
            assert len(disasm) > 0
